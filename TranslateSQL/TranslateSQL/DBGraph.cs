...
namespace TranslateSQL
{
    ...
    public class DBGraph
    {
        ...
        private string GetConnectionString(string dbName, string dataSourceName)
        {
            return $@"Data Source = .\{dataSourceName}; Initial Catalog = {dbName}; Integrated Security = True";
        }

        public static DBGraph CreateGraph(string dbName, string dataSourceName)
        {
            DBGraph graph = new DBGraph();
            string connectionString = graph.GetConnectionString(dbName, dataSourceName);
            ...
            using (SqlConnection sqlConnection = new SqlConnection(connectionString))
            {
                ...
                {
                    tableName = row.ItemArray[allTablesSchemaTable.Columns[2].Ordinal].ToString();
